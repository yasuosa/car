...
<body class="childrenBody">
	...
	<form class="layui-form" method="post" id="searchFrm">
		<div class="layui-form-item">
		    ...
		    <div class="layui-inline">
		      <button type="button" class="layui-btn layui-btn-normal  layui-icon layui-icon-search layui-btn-sm" id="doSearch">查询</button>
		      <button type="reset" class="layui-btn layui-btn-warm  layui-icon layui-icon-refresh layui-btn-sm">重置</button>
		    </div>
		 </div>
	</form>
	    ...
	    layui.extend({
			 ...
			 tableIns=table.render({
				...
				,done: function(res, curr, count){
					...
					if(res.data.length==0){
					    curr=curr-1;
					    console.log(curr)
					    tableIns.reload({url:"${pageContext.request.contextPath}/menu/loadAllMenu.action",page:{curr:curr }});
					}
				}
			})
			//模糊查询
		   ...
		   table.on('tool(menuTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
			      ...
			      $.post("${pageContext.request.contextPath}/menu/checkMenuHasChildren.action?id="+data.id,
					function(obj) {
						if(obj.code>=0){
							layer.msg("当前菜单有子节点,请先删除子节点");
						}else {
							layer.confirm('真的删除【'+data.title+'】行么', function(index){
							//向服务端发送删除指令
			...
			//向服务端发送删除指令
			$.post("${pageContext.request.contextPath}/menu/deleteBatchMenu.action", params, function (res) {
				layer.msg(res.msg);
					tableIns.reload();
					})
				});
